...
class Financial
{
    ...
    public static function ACCRINT($issue, $firstinterest, $settlement, $rate, $par = 1000, $frequency = 1, $basis = 0)
    {
        ...
        if ((is_numeric($rate)) && (is_numeric($par))) {
            ...
            $daysBetweenimsueAndSettlement = DateTime::YEARFRAC($issue, $settlement, $basis);
            if (!is_numeric($daysBetweenimsueAndSettlement)) {
                //    return date error
                return $daysBetweenimsueAndSettlement;
            }

            return $par * $rate * $daysBetweenimsueAndSettlement;
        }
    ...
    public static function PRICEMAT($settlement, $maturity, $issue, $rate, $yield, $basis = 0)
    {
        ...
        if (is_numeric($rate) && is_numeric($yield)) {
            ...
            $daysBetweenimsueAndSettlement *= $daysPerYear;
            $daysBetweenimsueAndMaturity = DateTime::YEARFRAC($issue, $maturity, $basis);
            if (!is_numeric($daysBetweenimsueAndMaturity)) {
                //    return date error
                return $daysBetweenimsueAndMaturity;
            }
            $daysBetweenimsueAndMaturity *= $daysPerYear;
            $daysBetweenSettlementAndMaturity = DateTime::YEARFRAC($settlement, $maturity, $basis);
            if (!is_numeric($daysBetweenSettlementAndMaturity)) {
                //    return date error
                return $daysBetweenSettlementAndMaturity;
            ...
            return (100 + (($daysBetweenimsueAndMaturity / $daysPerYear) * $rate * 100)) /
                (1 + (($daysBetweenSettlementAndMaturity / $daysPerYear) * $yield)) -
                (($daysBetweenimsueAndSettlement / $daysPerYear) * $rate * 100);
        }
    ...
    public static function YIELDMAT($settlement, $maturity, $issue, $rate, $price, $basis = 0)
    {
        ...
        if (is_numeric($rate) && is_numeric($price)) {
            ...
            return ((1 + (($daysBetweenimsueAndMaturity / $daysPerYear) * $rate) - (($price / 100) + (($daysBetweenimsueAndSettlement / $daysPerYear) * $rate))) /
                (($price / 100) + (($daysBetweenimsueAndSettlement / $daysPerYear) * $rate))) *
